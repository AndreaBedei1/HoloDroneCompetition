...
\end{equation}
where $d_{ij}$ is the inter-vehicle distance, $c$ the speed of sound and $\tau_{\mathrm{proc}}$ a fixed processing delay. Each delivery is also dropped with probability $p_{\mathrm{loss}}$. Payloads are capped to a small size to model the low bandwidth and a vehicle may not transmit faster than a configured per-sender interval. Packet loss is drawn from a seeded generator, so a run remains reproducible.

This implemented acoustic-inspired transport is not a fully characterized channel model. It provides the substrate for the leader-follower coordination of Section~\ref{sec:coordination} and is exercised as part of that coordination validation. Its parameters (packet loss, maximum range, latency and freshness window) are not independently swept.

\subsection{Leader-Follower Coordination}
\label{sec:coordination}

A staggered team of identical controllers remains separated in time, whereas a heterogeneous team does not. A faster follower released behind a slower leader catches up, causing the vehicles to bunch at gates where they converge on the same aperture center. We therefore provide a coordination controller that uses only legal information to maintain a spaced convoy. It wraps, rather than replaces, a gate-passing controller that exposes a \cmd{LocalCourseTracker}, keeping coordination and gate navigation separate. Both official camera-assisted controllers satisfy this interface.
...
The policy reads only the official observation, its own tracker, the static predecessor assignment and controller-authored messages; referee progress can disagree with either local estimate but never changes a coordination decision.
