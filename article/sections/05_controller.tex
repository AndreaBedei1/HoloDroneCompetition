...
The reference controller, \emph{Continuous Servo} (\cmd{rule\_gate\_baseline}), is deliberately interpretable rather than optimal. Its role is to provide a deterministic policy that is exercised across the clean tracks. Obstacle construction is validated separately and no obstacle-avoidance capability is claimed. Let $\beta$ and $\rho$ be the beacon bearing and range, $d$ the measured depth and $d^\star$ the nominal hold depth. The policy is organized into the four control components below; together they drive the shared \textsc{LocalCourseTracker} state machine of Fig.~\ref{fig:rule_baseline_state}.

\begin{itemize}
\item \emph{Bearing alignment and surge gating.} The controller aligns the vehicle before allowing sustained forward motion. Yaw tracks the beacon bearing through a clamped proportional law with a small deadband,
\begin{equation}
\label{eq:rule_yaw}
...
The second controller, \emph{Center-then-Commit} (\cmd{rule\_gate\_center\_then\_commit}), changes the gate-passage command rather than the observation, tracker or command envelope. It uses the same received beacon packets, front camera, depth and DVL feedback, command caps, smoothing and local confirmation logic as Continuous Servo (\cmd{rule\_gate\_baseline}). During \textsc{commit} and \textsc{verify\_exit}, Continuous Servo continues bounded lateral and vertical image correction while damping yaw from the IMU angular-rate measurement. Center-then-Commit instead stops chasing image-centroid changes: it sets image-driven sway to zero, holds the depth measured at lock, damps yaw from the same IMU angular-rate measurement and permits only a small acoustic lateral correction while maintaining forward surge. Let $q_t$ and $a_t$ be the camera-target confidence and image-area fraction and let $(c_{x,t},c_{y,t})$ be the normalized image errors. The normal lock condition is
\begin{equation}
