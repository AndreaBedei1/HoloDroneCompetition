...
\end{cases}
\end{equation}
where $f_i=1$ denotes a finished vehicle, $G_i^{\mathrm{done}}$ is the number of completed gates, $n_i^{\mathrm{coll}}$ the collision count and $\mathrm{dist}_i$ the distance to the next gate; the participant id is the deterministic final tie-break. Thus finished vehicles sort by penalized time, while unfinished vehicles sort by progress, then fewer collisions, then fewer penalties, then proximity to the next gate, so partial runs remain informative rather than being discarded.
