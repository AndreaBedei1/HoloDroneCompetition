...
\end{equation}
with world-up $\hat{z}=(0,0,1)$; if the passage direction is vertical ($\lVert\hat{z}\times\hat{n}\rVert\!\approx\!0$), $\hat{r}_g$ falls back to the world $\hat{x}$ axis so the frame stays well defined. For consecutive referee-side positions $p_{t-1},p_t$, the signed distances to the gate plane are
\begin{equation}
