...
\end{tikzpicture}
\caption{Team-level evaluation of a two-rover staggered fleet. Each rover is active from its release to its finish; the team elapsed time spans the earliest release to the latest finish (\eqref{eq:team_time}) and the team summary aggregates gates, penalties and inter-vehicle events (\eqref{eq:team_gates} to \eqref{eq:team_pen}). Per-rover rows remain diagnostics; the fleet competes as one team rather than against itself.}
