...
\foreach \x/\lab in {0/{$T_1^{\mathrm{release}}=0$}, 1.8/{$T_2^{\mathrm{release}}=g$}} {
  \draw (\x,0.12) -- (\x,-0.12);
}
\node[font=\scriptsize, anchor=north] at (0,-0.32) {$T_1^{\mathrm{release}}=0$};
\node[font=\scriptsize, anchor=north] at (1.95,-0.32) {$T_2^{\mathrm{release}}=g$};
\end{tikzpicture}
\caption{Team-level evaluation of a two-rover staggered fleet. Each rover is active from its release to its finish; the team elapsed time spans the earliest release to the latest finish (\eqref{eq:team_time}), and the team summary aggregates gates, penalties, and inter-vehicle events (\eqref{eq:team_gates}--\eqref{eq:team_pen}). Per-rover rows remain diagnostics; the fleet competes as one team rather than against itself.}
