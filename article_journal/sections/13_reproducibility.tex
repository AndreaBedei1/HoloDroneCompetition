...
Every input these commands read is the released evidence package
\cmd{artifacts/paper/}, which holds the per-run benchmark rows, the current-free
evaluation, the frozen policy checkpoint with its evaluation episodes and the
perception audit. Running \cmd{verify\_claims.py} recomputes each reported
quantity and fails on any mismatch; running \cmd{regenerate\_tables.py} rewrites
the seven data-driven tables byte-identically and verifies the penalty identity
