...
The released evidence package provides the artifacts, configuration and
provenance used to verify the reported results. This section documents them so a
reader can verify a value, re-derive a table, or run an equivalent experiment.
...
name without modifying the package (Section~\ref{sec:controller}). The two
reference controllers appear under the registry aliases
\cmd{rule\_gate\_baseline} for Continuous Servo and
\cmd{rule\_gate\_center\_then\_commit} for Center-then-Commit. Reproducing a
...
\mra{} is a configurable benchmark framework rather than a hosted leaderboard
with a hidden test set: there is no submission service, and evaluation is run
locally. Users are not restricted to the official circuits released here, and can
define private or custom circuits through the same configuration schema and score
them with the same referee.
...
seeded generators. For the learning extension, a seed registry committed to the
repository records the seed ranges each experiment has consumed, so that a later
experiment does not reuse them. Each evaluation records both requested and
completed seeds.
...
# Every quantitative claim in the text and the tables
python article_journal/scripts/verify_claims.py

# Data-driven result tables and the penalty-identity check
python article_journal/scripts/regenerate_tables.py

# Track layouts and the controller-comparison plot
python article_journal/scripts/generate_figures.py

# Perception panels from the released captures
...
These commands read the released evidence package \cmd{artifacts/paper/}, which
holds the per-run benchmark rows, the current-free evaluation, the frozen policy
checkpoint with its evaluation episodes and the perception audit, together with
committed track configurations and tooling. Running \cmd{verify\_claims.py}
recomputes each reported quantity and fails on any mismatch; running
\cmd{regenerate\_tables.py} rewrites the seven data-driven tables
byte-identically and verifies the penalty identity
$T^{\mathrm{pen}}=T^{\mathrm{official}}+P$ on every finished run. The two figure
scripts write a provenance JSON alongside each output recording their input
...
The learning section is deliberately structured so that its evaluation can be
refreshed without changing the benchmark protocol. A new frozen checkpoint is
added by recording its selection under the same criterion, running the same three
seeds per official circuit, and regenerating Table~\ref{tab:learning} from the
per-episode result files; since that criterion selects on the circuits the
checkpoint is then scored on, the resulting table is not a generalization result.
The methodology of Section~\ref{sec:learning} --- the 27-D observation encoding,
recurrent architecture and reward --- is independent of any one checkpoint.
