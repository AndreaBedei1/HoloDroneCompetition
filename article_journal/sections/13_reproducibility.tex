...
The learning section is deliberately structured so that its tables can be
refreshed without rewriting the manuscript. A new frozen checkpoint is added by
recording its SHA-256 in the benchmark package manifest, running the same matched
suite on the same registered seeds, and regenerating
Tables~\ref{tab:learning}--\ref{tab:learning_paired} and
Fig.~\ref{fig:learning_group_success} from the resulting aggregate files. The
methodology of Section~\ref{sec:learning} --- observation encoding, reward,
curriculum, readiness gate and evaluation protocol --- is independent of any
particular checkpoint and does not change when the numbers do.
