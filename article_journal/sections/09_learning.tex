...
released provenance; its own training budget is not part of that figure. The
observation representation and control interface are the ones described above and
...
and frozen before the reported episodes, with the full selection record released
in the repository provenance, and the controller is then executed without
additional learning or online adaptation. The complete hyperparameter, reward and
checkpoint configuration is released with the repository. Attempts that produced
no valid episode because of a simulator or engine fault rather than controller
behaviour were recorded as technical-invalid, then excluded and repeated; the
reported PPO results comprise nine valid evaluation episodes, and the released
artifacts include one such technical-invalid attempt caused by an engine startup
failure. The results of this fixed policy are reported together with the other
benchmark evaluations in Section~\ref{sec:eval_rq5}.
