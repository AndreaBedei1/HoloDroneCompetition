...
A racing benchmark is only well posed if target selection remains part of the
...
colour or brightness segmentation was not used here because underwater
rendering can substantially alter their apparent intensity relative to the
water and seabed. The detector therefore relies primarily on geometric
structure: edges are extracted from the grayscale image, closed contours are
converted into
...
oblique views, recorded separately from the audit frames of
Section~\ref{sec:perception_audit}, a valid pose estimate was obtained in only
$11.7\%$ of frames and the median plane-yaw error was $48.2^\circ$. Both figures
therefore describe strongly oblique viewing only. The estimator is unreliable
under large oblique viewing angles, motivating the separate characterization
under the near-frontal viewing conditions encountered during normal racing.


\subsection{Diagnostic Perception Audit on the Official Circuits}
\label{sec:perception_audit}

This subsection reports a diagnostic audit of the perception front end rather
than a statistical validation of it. To characterize perception under the
conditions encountered by the reference controllers, we recorded 60 frames per
circuit along the trajectories of the Center-then-Commit reference controller
under current-free conditions on the three official circuits. The sample is
therefore small and covers one controller and one disturbance setting; the
proportions below describe the recorded frames and are reported descriptively.
Ground truth was used only offline to evaluate the perception output and never
entered the control loop. Table~\ref{tab:perception_audit} summarizes detection
availability, quadrilateral recovery, pose availability and multi-candidate
target association, while
...
frames, against the $88$--$92\%$ of the same frames in which a gate centre was
detected. The reference controllers are consequently designed to tolerate
intermittent visual measurements rather than assume that a complete gate
geometry is available at every control step.

Frames containing multiple visual candidates were rare in this sample, so the
association counts are correspondingly small. In every recorded frame
...
observed in the inspected multi-candidate cases. These counts are far smaller
than the number of frames underlying the detection rates above, and are reported
as a consistency check on the inspected cases rather than as a comparison with
them. In these frames the acoustic bearing was sufficient to resolve identity
when several geometrically plausible gates were visible.

The pose entries of Table~\ref{tab:perception_audit} and the oblique-view
figures of Section~\ref{sec:perception_pose} were obtained on different frame
sets and describe different viewing-geometry regimes; they are not two estimates
of the same quantity. The oblique-view figures come from views deliberately
selected to be strongly oblique, whereas the frames audited here were recorded
along reference trajectories on which the controller approaches the gate
approximately frontally. In this near-frontal regime the recovered plane
orientation was stable, with median yaw error below
$1.2^\circ$ on all three circuits and no gross orientation or sign errors
observed among the inspected frames. Under strongly oblique views, by contrast,
pose recovery became sparse and inaccurate. The relevant conclusion is therefore
not that the pose estimator is generally reliable, but that its accuracy depends
strongly on viewing geometry.

For the racing task considered here, reliable gate-centre detection and
acoustic--visual association were sufficient for the reference controllers in
these runs, while full pose recovery is substantially more sensitive to
viewpoint. The diagnostic audit characterizes these components independently
from the course-completion results reported in Section~\ref{sec:evaluation}.
