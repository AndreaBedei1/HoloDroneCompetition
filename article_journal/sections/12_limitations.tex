...
restricted to the \texttt{medium} condition on one circuit (Horseshoe Bay).
A broader range of current magnitudes, and the same evaluation on the other two
circuits, would be needed to characterize the degradation curve more fully.
...
but the systematic benchmark evaluation has not yet been exercised against
structurally different autonomy strategies, such as a model-predictive controller, a cascaded design
with an inner DVL velocity loop, or a planner that reasons about several gates
ahead. The separate recurrent-PPO validation demonstrates interface
...
The learned-control evidence is a limited validation snapshot. A recurrent
learned controller can be integrated and evaluated through the common interface,
but the experiment does not establish superiority, robustness under currents,
fleet-level learning, sample efficiency or transfer to unseen circuits. The
validation sample is small, and the learning protocol is not factorially matched
to the rule-controller evaluation. Broader comparisons of recurrent architectures,
