"""Onboard-only temporal context for the 27-D Gen-2 contract."""
...
class OnboardLocalTransition27dContextTracker(OnboardLocalTransitionContextTracker):
    """Onboard-only temporal state tracker for the 27-D contract.

    Integrates:
    - Proven causal TemporalVisionTracker via the base local tracker;
    - Acoustic beacon association with the locally expected gate;
    - Correct gate center and aperture corner detection;
    - Gate pose estimation and yaw filter from :mod:`gate_pose`;
    - Immediate pose filter reset on expected-beacon changes preventing orientation leakage;
    - Orientation emitted strictly when the visual target is locked and unpredicted;
    - Zero ground truth, world pose, referee progress, or future geometry.
    """
    ...
    def gate_orientation_age_steps(self) -> int:
        """Missed camera frames behind current filtered estimate."""
    ...
    def reset(self, first_observation: Optional[Mapping[str, Any]] = None) -> None:
        ...
        self._last_context = LocalTransition27dLearningContext(
            expected_beacon_id=base.expected_beacon_id,
            depth_reference_m=base.depth_reference_m,
            visual_target=base.visual_target,
            use_tracked_visual_target=base.use_tracked_visual_target,
            prev_action=base.prev_action,
            vision_center_x_rate_s=base.vision_center_x_rate_s,
            vision_center_y_rate_s=base.vision_center_y_rate_s,
            vision_rate_valid=base.vision_rate_valid,
            beacon_bearing_rate_deg_s=base.beacon_bearing_rate_deg_s,
            beacon_elevation_rate_deg_s=base.beacon_elevation_rate_deg_s,
            beacon_range_rate_m_s=base.beacon_range_rate_m_s,
            beacon_rate_valid=base.beacon_rate_valid,
            target_changed_recently=base.target_changed_recently,
            time_since_target_change_s=base.time_since_target_change_s,
    ...
    def context(
        ...
        *,
        ...
        if base.expected_beacon_id != expected_before:
            # Target switch resets gate pose filter immediately
            self._pose_tracker.reset()
